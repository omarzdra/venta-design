...
async function request(path, options = {}) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(data?.message || data?.error || "Prislo je do napake na strezniku.");
    return data;
  } catch (error) {
    if (error.name === "AbortError") throw new Error("Zahteva je potekla. Poskusi znova.");
    throw error;
  } finally {
