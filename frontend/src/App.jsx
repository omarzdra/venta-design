...
const calcStoritveTotal = (rows, storitve) => (rows || []).reduce((sum, row) => {
  ...
  return sum + Number(row.stevilo_ur || 0) * Number(storitev?.eur_ura || 0);
}, 0);
...
function NalogaForm({ lots, storitve, initial, role, onSave, onCancel, notify }) {
  const storageKey = (initial?.tip || "splosno") === "vozila" ? "nalogaVozila" : "nalogaSplosno";
  ...
  useEffect(() => {
    ...
    const key = form.tip === "vozila" ? "nalogaVozila" : "nalogaSplosno";
  ...
  const total = materialTotal + storitveTotal + dnevniTotal + Number(form.cena_dela_neto || 0);
  ...
  async function submit(e) {
    ...
    try {
      await onSave(form);
      if (!isEdit) {
        clearForm(form.tip === "vozila" ? "nalogaVozila" : "nalogaSplosno");
  ...
  return <form className="panel form-grid" onSubmit={submit}>
    ...
    <label>Stevilka racuna<input value={form.stevilka_racuna || ""} disabled={!admin} onChange={(e) => set({ stevilka_racuna: e.target.value })} /></label>
    ...
    {admin && <div className="form-section wide"><h3>Cena dela</h3><div className="form-grid"><label>Cena dela neto (EUR)<input type="number" step="0.01" value={form.cena_dela_neto || ""} onChange={(e) => set({ cena_dela_neto: e.target.value })} /></label><label>DDV<select value={form.ddv_stopnja || 22} onChange={(e) => set({ ddv_stopnja: e.target.value })}><option value={0}>0%</option><option value={9.5}>9.5%</option><option value={22}>22%</option></select></label><label>Bruto (EUR)<input readOnly value={money(Number(form.cena_dela_neto || 0) * (1 + Number(form.ddv_stopnja || 22) / 100))} /></label></div></div>}
    ...
    <div className="wide"><MaterialRows rows={form.materiali || []} setRows={(materiali) => set({ materiali })} lots={lots} /></div>
    <div className="form-section wide"><div className="section-head"><h3>Storitve</h3><button type="button" className="btn secondary" onClick={addStoritev}>+ Dodaj storitev</button></div>{(form.storitve || []).map((s, i) => <div className="material-row" key={i}><SearchSelect options={storitve.map((sv) => ({ value: sv.id, label: sv.naziv, sub: `${sv.eur_ura} EUR/uro` }))} value={s.storitev_id} onChange={(val) => updateStoritev(i, { storitev_id: val })} placeholder="Izberi storitev..." /><input type="number" step="0.01" value={s.stevilo_ur} onChange={(e) => updateStoritev(i, { stevilo_ur: e.target.value })} placeholder="Stevilo ur" /><span>{money(calcStoritveTotal([s], storitve))}</span><button type="button" className="icon-btn danger" onClick={() => removeStoritev(i)}>x</button></div>)}</div>
    <div className="form-section wide"><h3>Dnevni strosek</h3><div className="form-grid"><label>Dnevni strosek (EUR/dan)<input type="number" step="0.01" value={form.dnevniStrosek?.dnevni_strosek || ""} onChange={(e) => set({ dnevniStrosek: { ...form.dnevniStrosek, dnevni_strosek: e.target.value } })} /></label><label>Stevilo dni<input type="number" min="1" value={form.dnevniStrosek?.stevilo_dni || ""} onChange={(e) => set({ dnevniStrosek: { ...form.dnevniStrosek, stevilo_dni: e.target.value } })} /></label><label>Skupaj<input readOnly value={money(dnevniTotal)} /></label></div></div>
    <div className="wide"><ImagesInput value={form.slike || []} onChange={(slike) => set({ slike })} notify={notify} /></div>
    <section className="kpi-row wide"><div className="kpi"><span>Material</span><strong>{money(materialTotal)}</strong></div><div className="kpi"><span>Storitve</span><strong>{money(storitveTotal)}</strong></div><div className="kpi"><span>Dnevni strosek</span><strong>{money(dnevniTotal)}</strong></div>{admin && Number(form.cena_dela_neto || 0) > 0 && <div className="kpi"><span>Cena dela</span><strong>{money(form.cena_dela_neto)}</strong></div>}</section>
    <div className="total-line wide">Predvidena vrednost projekta: <strong>{money(total)}</strong></div>
    <div className="form-actions wide">{onCancel && <button type="button" className="btn secondary" onClick={onCancel}>Preklici</button>}<button className="btn primary" disabled={!valid || isSubmitting}>{isEdit ? (isSubmitting ? "Shranjujem..." : "Shrani spremembe") : (isSubmitting ? "Ustvarjam..." : "Ustvari nalogo")}</button></div>
  </form>;
}
...
function InventuraModal({ onClose, notify }) {
  ...
  const load = useCallback(() => api.inventure().then((items) => { setInventure(items); if (selected) setSelected(items.find((i) => i.id === selected.id) || null); }), [selected]);
...
function PonudbaView({ produkti, lots, storitve, setPreneseniPodatki, setTab }) {
  ...
  const totalCost = nabavna + storitveTotal + dnevniTotal;
...
export default function App() {
  ...
  const boot = useCallback(async function boot(session, showLoading = false, markAuthChecked = false) { if (showLoading) setLoading(true); try { if (session) { await loadUser(); await loadData().catch((e) => notify(e.message, "error")); } else { setUser(null); } } catch { setUser(null); } finally { if (showLoading) setLoading(false); if (markAuthChecked) setAuthChecked(true); } }, [loadData, loadUser, notify]);
