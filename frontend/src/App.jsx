...
function missingConfirmFields(naloga) {
  ...
  if (Number(naloga.cena_dela_neto || 0) <= 0) missing.push("cena");
...
function productTypeLabel(tip) {
  if (tip === "adr") return "ADR/Oprema";
  if (tip === "tabla") return "Tabla";
  return "Folija";
}
...
function ProductFormModal({ title = "Dodaj material", onClose, onSaved }) {
  const [form, setForm] = useState({ koda: "", naziv_produkta: "", tip: "folija", sirina: "", nabavna_cena: "", prodajna_cena: "", dobavitelj: "", zacetna_kolicina: "" });
  async function save() {
...
function NalogaForm({ lots, storitve, initial, role, onSave, onConfirm, onCancel, notify }) {
  ...
  const valid = form.naziv_projekta && !overStock && (form.tip !== "vozila" || form.vozilo?.znamka_vozila);
  async function submit(e) {
    e.preventDefault();
  ...
  async function confirmNaloga() {
    const missing = missingConfirmFields(form);
    if (missing.length) {
      notify?.(`Za potrditev dopolni: ${missing.join(", ")}.`, "error");
      return;
    }
    setIsSubmitting(true);
    try {
  ...
  return <form className="panel form-grid" onSubmit={submit}>
    ...
    <label>Status<select value={form.status} disabled={!admin || !isEdit} onChange={(e) => set({ status: e.target.value })}><option value="v_izdelavi">V izdelavi</option><option value="dokoncana">Dokoncano</option><option value="ni_realizirano">Ni realizirano</option>{isEdit && <option value="potrjena">Potrjeno</option>}</select></label>
    <label>Stevilka racuna<input value={form.stevilka_racuna || ""} disabled={!admin} onChange={(e) => set({ stevilka_racuna: e.target.value })} />{admin && isEdit && !["potrjena", "ni_realizirano"].includes(form.status) && <button type="button" className="btn primary inline-action" disabled={isSubmitting || !onConfirm} onClick={confirmNaloga}>Potrdi nalog</button>}</label>
    <label className="wide">Opis storitve<textarea value={form.opis || ""} onChange={(e) => set({ opis: e.target.value })} /></label>
    <label>Ime narocnika<input value={form.kontakt_ime || ""} onChange={(e) => set({ kontakt_ime: e.target.value })} /></label><label>GSM<input value={form.kontakt_gsm || ""} onChange={(e) => set({ kontakt_gsm: e.target.value })} /></label><label>Email<input type="email" value={form.kontakt_email || ""} onChange={(e) => set({ kontakt_email: e.target.value })} /></label>
      ...
      <div className="dense-list" style={{ marginTop: 12 }}>{filtered.map((p) => {
        ...
        return <article className="inventory-item" key={p.id}>
          <button className="item-main" onClick={() => setExpanded(expanded === p.id ? null : p.id)}>
            <div><strong>{p.koda} - {p.naziv_produkta}</strong><span className="muted">{p.dobavitelj || "Brez dobavitelja"}</span><span>{productTypeLabel(p.tip)} {p.sirina ? `- sirina ${p.sirina}` : ""}</span></div>
          ...
          {expanded === p.id && <div className="lot-list">{(p.lotProdukti || []).length === 0 ? <p className="muted">Ni aktivnih LOTov.</p> : p.lotProdukti.map((lot) => {
            return <div className="lot-row inventory-lot-row" key={lot.id}>
              ...
              <div className="lot-card-grid">{p.tip === "folija" ? <><div className="lot-mini-card"><span>Preostalo</span><strong>{Number(lot.kolicina_tm).toFixed(2)} tm</strong>{p.sirina && <small>{(Number(lot.kolicina_tm) * Number(p.sirina)).toFixed(2)} m2</small>}</div>{showCosts && <div className="lot-mini-card"><span>Nabavna cena</span><strong>{money(Number(lot.nabavna_cena))}/m2</strong>{p.sirina && <small>{money(Number(lot.nabavna_cena) / Number(p.sirina))}/tm</small>}</div>}{showCosts && <div className="lot-mini-card"><span>Prodajna cena</span><strong>{money(Number(lot.prodajna_cena))}/m2</strong>{p.sirina && <small>{money(Number(lot.prodajna_cena) / Number(p.sirina))}/tm</small>}</div>}</> : p.tip === "tabla" ? <><div className="lot-mini-card"><span>Preostalo</span><strong>{Number(lot.kolicina_tm).toFixed(2)} m2</strong></div>{showCosts && <div className="lot-mini-card"><span>Nabavna cena</span><strong>{money(Number(lot.nabavna_cena))}/m2</strong></div>}{showCosts && <div className="lot-mini-card"><span>Prodajna cena</span><strong>{money(Number(lot.prodajna_cena))}/m2</strong></div>}</> : <><div className="lot-mini-card"><span>Preostalo</span><strong>{Number(lot.kolicina_tm).toFixed(0)} kos</strong></div>{showCosts && <div className="lot-mini-card"><span>Nabavna cena</span><strong>{money(Number(lot.nabavna_cena))}/kos</strong></div>}{showCosts && <div className="lot-mini-card"><span>Prodajna cena</span><strong>{money(Number(lot.prodajna_cena))}/kos</strong></div>}</>}{showCosts && <div className="lot-mini-card"><span>Marza</span><strong>{money(Number(lot.prodajna_cena || 0) - Number(lot.nabavna_cena || 0))}</strong><small>na enoto</small></div>}</div>
...
function DetailModal({ naloga, role, onClose, onEdit, reload, notify }) {
  ...
  const confirmMissing = missingConfirmFields(naloga);
  const canComplete = ["admin", "grega"].includes(role) && !["potrjena", "ni_realizirano"].includes(naloga.status);
  const canEditNaloga = ["admin", "grega"].includes(role) && naloga.status !== "potrjena";
  ...
  async function confirm() {
    if (confirmMissing.length) {
      notify(`Za potrditev dopolni: ${confirmMissing.join(", ")}.`, "error");
      return;
    }
    await api.potrdiNaloga(naloga.id, {});
    notify("Naloga je potrjena.");
  ...
  return <Modal title={`Podrobnosti nakupa - ${nakup.stevilka_racuna || "Brez stevilke"}`} onClose={onClose}>
    <form className="form-grid" onSubmit={save}>
      ...
      <table className="mobile-cards wide">
        ...
        const neto = p.kategorija === "material" ? Number(p.neto_cena || 0) : Number(p.kolicina || 0) * Number(p.cena_postavke || 0);
        const bruto = neto * (1 + Number(p.ddv || 0) / 100);
        return <tr key={p.id}>
          ...
          <td data-label="Kolicina">{p.kategorija === "material" ? `${Number(p.kolicina_tm || 0).toFixed(2)} ${unit}` : editing ? <input type="number" step="0.01" value={p.kolicina || ""} onChange={(e) => setForm({ ...form, postavke: form.postavke.map((row, idx) => idx === i ? { ...row, kolicina: e.target.value } : row) })} /> : Number(p.kolicina || 1).toFixed(2)}</td>
          <td data-label="Neto">{editing ? <input type="number" step="0.01" value={p.kategorija === "material" ? (p.neto_cena || "") : (p.cena_postavke || "")} onChange={(e) => setForm({ ...form, postavke: form.postavke.map((row, idx) => idx === i ? (row.kategorija === "material" ? { ...row, neto_cena: e.target.value } : { ...row, cena_postavke: e.target.value }) : row) })} /> : money(p.neto_cena)}</td>
...
function PonudbaView({ produkti, lots, storitve, setPreneseniPodatki, setTab, notify }) {
  ...
  async function savePonudba() {
    ...
    setSaveName("");
    setForm({ tip: "splosno", materiali: [], storitve: [], dnevniStrosek: { dnevni_strosek: "", stevilo_dni: "" } });
